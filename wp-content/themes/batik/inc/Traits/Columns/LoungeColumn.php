...
class LoungeColumn extends LoungeBase {

    public function loungeName( $item ) {
    ...
    public function loungeTransactionDate( $item ) {
    ...
    public function loungeDetail( $item ) {
        $editLink = $this->editLink( $item->lounge_id );
        $filterLink = $this->registrationFilterLink( $item->lounge_id );
        $room = $item->lounge;

        $el = '<a href="'. $editLink .'" class="gg-color-dte">'. $room->post_title .'</a>';
        $el .= '<div class="row-actions">
            <span class="lounge_filter">
    ...
    public function loungePrice( $item ) {
        $transaction = $item->transaction;
    ...
    public function loungePaid( $item ) {
        $transaction = $item->transaction;
    ...
    public function loungeActions( $item ) {
    ...
    public function transactionDate( $item ) {
        // Date Link
        $el = '<a href="'. $this->transactionDetailLink( $item->id ).'" title="View Transactions Details for TXN #'. $item->id .'">'. $item->timestamp->format( get_option('date_format') . ' ' . get_option('time_format') ).'</a>';

        // Status stamp
        $el .= '<br/><i>'. $item->textStatusColored .'</i>';

        return $el;
    }

    public function transactionRegistrant( $item ) {
        $registration = $item->lounge_registration;
    ...
    public function transactionLounge( $item ) {
        $registration = $item->lounge_registration;
        $room = $registration->lounge;

        // View Filter
        $el = '<a href="'. $this->editLink( $room->ID ).'" title="View Registration Details">'. $room->post_title .'</a>';

        if( empty($_REQUEST['lounge_id']) ) {
            $el .= '<div class="row-actions">
    ...
    public function transactionActions( $item ) {
        ...
        $el = '<ul class="reg-overview-actions-ul actions-ul">';
            ...
            // View Transaction Invoice
            $el .= '<li>
                <a title="View Transaction Invoice" target="_blank" href="'. get_report_invoice_link( $item->lounge_registration->reg_code, $item->id ).'" class="tiny-text">
                    <span class="dashicons dashicons-media-spreadsheet ee-icon-size-18"></span>
                </a>
            </li>';
            ...
            $el .= '<li>
                <a title="View Transaction Receipt" target="_blank" href="'. get_report_receipt_link( $item->lounge_registration->reg_code, $item->id ).'" class="tiny-text">
            ...
            $el .= '<li>
                <a href="'. $this->registrationDetailLink( $item->lounge_registration->id ) .'" title="View Registration Details" class="tiny-text">
                    <div class="dashicons dashicons-clipboard"></div>
                </a>
            </li>';
    ...
    public function contactRegistrationCount( $item ) {
        $filterLink = $this->contactFilterLink( $item->id );

        return '<a href="'. $filterLink .'" title="Filter registration by this contact">'. $item->lounge_registrations_count.'</a>';
    }
