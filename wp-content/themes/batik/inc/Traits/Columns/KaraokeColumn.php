...
class KarokeColumn extends KaraokeBase {

    public function roomName( $item ) {
    ...
    public function roomTransactionDate( $item ) {
    ...
    public function roomDetail( $item ) {
        $editLink = $this->editLink( $item->room_id );
        $filterLink = $this->registrationFilterLink( $item->room_id );
        $room = $item->room;

        $el = '<a href="'. $editLink .'" class="gg-color-dte">'. $room->post_title .'</a>';
        $el .= '<div class="row-actions">
            <span class="room_filter">
    ...
    public function roomPrice( $item ) {
        $transaction = $item->transaction;
    ...
    public function roomActions( $item ) {
    ...
    public function transactionRegistrant( $item ) {
        $registration = $item->room_registration;
    ...
    public function transactionActions( $item ) {
        ...
        $el = '<ul class="reg-overview-actions-ul actions-ul">';
            ...
            // View Transaction Invoice
            $el .= '<li>
                <a title="View Transaction Invoice" target="_blank" href="'. get_report_invoice_link( $item->room_registration->reg_code, $item->id ).'" class="tiny-text">
                    <span class="dashicons dashicons-media-spreadsheet ee-icon-size-18"></span>
                </a>
            </li>';
            ...
            $el .= '<li>
                <a title="View Transaction Receipt" target="_blank" href="'. get_report_receipt_link( $item->room_registration->reg_code, $item->id ).'" class="tiny-text">
            ...
            $el .= '<li>
                <a href="'. $this->registrationDetailLink( $item->room_registration->id ) .'" title="View Registration Details" class="tiny-text">
                    <div class="dashicons dashicons-clipboard"></div>
                </a>
            </li>';
    ...
    public function contactRegistrantCount( $item ) {
        ...
        return '<a href="'. $filterLink .'" title="Filter registration by this contact">'. $item->room_registrations_count.'</a>';
    }
